...

{
	...
	public void Clk_Employee_Status(String Status)
	{
		...
		WebElement Clk_Emp_St = wait.until(ExpectedConditions.elementToBeClickable(Employee_Status));
		Clk_Emp_St.click();
		
		//CONSTRUCT THE DYNAMIC XPATH TO FIND THE  STATUS OPTION
		By Status_Option = By.xpath("\"//span[text()='\"" + Status + "']");
	...
	//CREATING A RE-USABLE METHOD TO SELECT A SPECEFIC OPTION FORM THE INCLUDE DROPDOWN
			...
			
			{
				...
				//CONSTRUCT THE X-PATH TO CLICK ON THE SPECEFIC OPTION ON THE DROPDOWN
				//THIS X-PATH IS DYNAMIC
				
				By Status_Option = By.xpath("//span[text()='\"" +Status+"']");
				WebElement Click_Option = wait.until(ExpectedConditions.elementToBeClickable(Status_Option));
				Click_Option.click();
				Assert.assertTrue(Click_Option.isDisplayed(), "Failed To select the specific option");
				}
				catch(Exception Inc)
				{
		...
		//CREATING A RE-USABLE METHOD TO CLICK RANDOMLY ON ANY OPTION ON THE DROP DOWN "INLCUDE"
			
			public void RNd_Select_Include()
			{
				try
				{
					WebElement Click_Drop_Arrow = wait.until(ExpectedConditions.elementToBeClickable(Include));
					Click_Drop_Arrow.click();
					
					//FETCHinG ALL THE OPTIONS OF DROPDOWN IN AN ARRAY 
					
					List<WebElement> Rnd_Slt_Opt = driver.findElements(By.xpath("//*[@id=\"app\"]/div[1]/div[2]/div[2]/div/div[1]/div[2]/form/div[1]/div/div[4]/div/div[2]/div/div[2]"));
					
					Random Random_Click = new Random();
					int randomIndex = Random_Click.nextInt(Rnd_Slt_Opt.size());
					WebElement randomOption =Rnd_Slt_Opt.get(randomIndex);
					randomOption.click();
					
					Assert.assertTrue(randomOption.isDisplayed(),"Failed To Select Random Option from DorpDown");
					System.out.println("Randomly Selected Option is "+randomOption.getText());
				}
				catch (Exception Inc)
				{
					System.err.println("Error In Selecting The Option "+Inc.getMessage());
					throw Inc ;
				}
			...
			public void Slt_Supervisior_Name(String Partial_Name , String Full_name)
			{
				try
				{
					WebElement Click_Supervisor_Field = wait.until(ExpectedConditions.elementToBeClickable(Supervisor_Name));
					Click_Supervisor_Field.click();
					Click_Supervisor_Field.clear();
					Click_Supervisor_Field.sendKeys("Sai"); // EDIT THIS NAME AS THI IS DYNAMIC THIS cAN BE CHANGED
					
					By Find_Option = By.xpath("//div[@class='oxd-form-actions']"+Full_name+"']"); //ADJUST LOCATOR IF NEEDED
					WebElement Slt_Frm_Drp = wait.until(ExpectedConditions.elementToBeClickable(Find_Option));
			...
			//CREATING A RE-USABLE METHOD TO SElECt A SPECIF OPTION FROM THE JOB TITLE DROP DOWN 
			
			public void Click_Spc_Opt_JobTitle()
			{
				try
				{
					WebElement Clk_Drp_Dwn = wait.until(ExpectedConditions.elementToBeClickable(Job_Title));
					Clk_Drp_Dwn.click();
					
					By Locate_Option = By.xpath("//div[@class='orangehrm-background-container']");	
					WebElement Clk_opt = wait.until(ExpectedConditions.elementToBeClickable(Locate_Option));
					Clk_opt.click();
					
					Assert.assertTrue(Clk_opt.isDisplayed(), "Failed to Select Option Form The Dropdown");
					System.out.println();
			...
			public void Rnd_Opt_Job_Title ()
			{
				...
				WebElement Open_DropDown = wait.until(ExpectedConditions.elementToBeClickable(Job_Title));
				Open_DropDown.click();
				
				List<WebElement> Fetch_All_Options = driver.findElements(By.xpath("//*[@id=\"app\"]/div[1]/div[2]/div[2]/div/div[1]/div[2]/form/div[1]/div/div[6]/div/div[2]/div/div[2]"));
				
				Random Select_Random_JobTitle = new Random();
				int randomIndex = Select_Random_JobTitle.nextInt(Fetch_All_Options.size());
				
				WebElement Click_Random_JobTitle = Fetch_All_Options.get(randomIndex);
				Click_Random_JobTitle.click();
				
				System.out.println("Randomly Selected Job Title "+Click_Random_JobTitle.getText());
				
				}
				catch(Exception Rnd_Jb_Title)
				{
					System.err.println("Unable To Select The Options The Error Message:"+Rnd_Jb_Title.getMessage() );				
			...
			
			{
				try 
				{
				WebElement clkDrpdwn = wait.until(ExpectedConditions.elementToBeClickable(Sub_Unit));
				clkDrpdwn.click();
				
				By LocateOption = By.xpath("//*[@id=\"app\"]/div[1]/div[2]/div[2]/div/div[1]/div[2]/form/div[1]/div/div[7]/div/div[2]/div/div[2]/div[4]");
				WebElement sltOption = wait.until(ExpectedConditions.elementToBeClickable(LocateOption));
				sltOption.click();
				
				Assert.assertTrue(sltOption.isDisplayed(),"Failed To Select The Expected Option");
			...
			//CREATING A RE USABLE METHOD TO SELECT A RANDOM OPTION FROM THE SUB UNIT FIELD
			
			public void sltRndSubUnit()
			{
				try 
				{
					WebElement openDrpDwn = wait.until(ExpectedConditions.elementToBeClickable(Sub_Unit));
					openDrpDwn.click();
					
					List<WebElement> fetchAllOption = driver.findElements(By.xpath("//*[@id=\"app\"]/div[1]/div[2]/div[2]/div/div[1]/div[2]/form/div[1]/div/div[7]/div/div[2]/div/div[2]"));
				
					Random indexOptions =new Random();
					int RandomIndex = indexOptions.nextInt(fetchAllOption.size());
					
					WebElement randomOption = fetchAllOption.get(RandomIndex);
					randomOption.click();
					
					System.out.println("The Selected Option From The Dropdowm is : "+randomOption.getText());
			...
			public void clkSearchButton ()
			{
				...
				try
				{
					WebElement clickSearch = wait.until(ExpectedConditions.elementToBeClickable(Search_Button));
					clickSearch.click();
					
					System.out.println("Clicked On the Search Button");
				}
				catch(Exception SrBT)
				{
			...
			public void clkResetButton()
			{
				...
				try
				{
				WebElement clkReset = wait.until(ExpectedConditions.elementToBeClickable(Reset_Button));
				clkReset.click();
			...
			public void clkADD()
			{
				try
				{
					WebElement clickADD = wait.until(ExpectedConditions.elementToBeClickable(Add_Button));
					clickADD.click();
					
					System.out.println("The Button Has Been CLicked ");	
				}
				
				catch(Exception ckADD)
				{
					System.err.println("The Error MESSAGE :" +ckADD.getMessage());
				}
			}
		
			
			
			public void sltAllRecord()
			{
				try
				{
					
					WebElement clickAllRd = wait.until(ExpectedConditions.elementToBeClickable(Select_All_ID));
					clickAllRd.click();
					
					System.out.println("SUCCESSFULY Selected ALl Records ");
				}
				catch(Exception ckALRd)
				{
					System.err.println("The errror message :"+ckALRd.getMessage());
				}
			}
			
			
			
			//CREATING RE - USABLE METHOD TO CLICK ON THE EDIT BUTTON OF A SPECIFIC RECORD EDIT BUTTON
			
			public void clkEditBtn()
			{
				//DON'T FORGOT TO CHNAGE THE LOCATOR OF THE EDIT BUTTON AS IT IS FOR THE SPECIFC EMPLOYEE
				WebElement clkEdit = wait.until(ExpectedConditions.elementToBeClickable(Edit));
				clkEdit.click();
		
			
			}

			//CREATING A RE USBALE METHOD TO CLICK ON THE EDIT BUTTON RANDOMLY FOR ANY OPTION .
			...
			{
				WebElement clickDelete = wait.until(ExpectedConditions.elementToBeClickable(Delete));
				clickDelete.click();
			}
						...
						if(url!=null && !url.isEmpty())
{
						...
						{
							System.out.println("The Link are Either Not Configured or  empty");
				...
				catch(Exception Brklink)
				{
					 System.err.println("Error while checking links: " + Brklink.getMessage());
				}
			}
			...
			public void vrfLinks(String urlLink)
			{
				...
				try
				{
					...
					{
						System.out.println("The Printed link is a Valid Link "+urlLink);
					}
				...
				catch(Exception vrflnk)
				{
					System.err.println("Error while checking links: " + vrflnk.getMessage());
